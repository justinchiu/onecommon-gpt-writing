...
\title{Modeling Perspective-Dependent Ambiguity \\ in Grounded Collaborative Dialogue}
...
\maketitle
\begin{abstract}
Errors in reference generation and resolution can occur if
a dialogue agent reasons incorrectly about their partner's perspective.
We present, in a collaborative and visually grounded setting,
a dialogue planner that infers its partner's perspective
to produce referring expressions that the partner resolves correctly.
The dialogue planner models the partner perspective as a latent variable,
embedded in a partner model that is used for both model-based planning
and incorporating evidence from partner responses. 
We validate our approach on \textsc{OneCommon}, 
a challenging dialogue game where players have large differences in their perspectives.
In symbolic selfplay, where agents partner with a copy of themselves
using symbolic communication,
the dialogue planner has promising performance,
winning 85.2\% of games.
%In automatic evaluation of plan proposals,
%reasoning about the partner's perspective as a latent variable
%results in a 12\% reduction in the total number of incorrectly
%resolved plans compared to a state-of-the-art baseline \citep{fried}.
\end{abstract}

%\justin{Perspective-dependent ambiguity may be too general. Covers different field of view AND other differences in perspective, such as different feature processing such as colorblindness. Either specify a field-of-view specific ambiguity, or clarify which perspective-dependent ambiguity subtype we focus on. For feature swap experiment: swap black and white wp .5}

\section{Introduction}
%\textbf{Collaboration is impeded by ambiguity}
Ambiguity is a key impediment to collaborative dialogue that makes
linguistics actions, such as reference resolution and generation,
 difficult to execute.
One cause of ambiguity in a grounded setting is differences in perspectives \citep{brownschmidt2011perspective}.
Agents that incorrectly assume their partner shares their perspective  may make errors in reference generation and resolution.
We refer to ambiguity induced by different perspectives as \textit{perspective-dependent ambiguity}.
...
% The game terminates after both agent and partner have selected a dot.
% The game is a success if they select the same dot and a failure otherwise.
% The different but overlapping views is the source of perspective-ambiguity,
...
% whose degree depends on the number of overlapping dots.
...
% Unknown to both the agent and partner, they both confirm.
...
% ignoring perspective-dependent ambiguity altogether.
% The egocentric heuristic 
...
%\textbf{We deal with perspective-dependent ambiguity}
%\justin{Our second contribution}
%\justin{TOO VAGUE. start with what you have, end up with specific solution}
...
%Incorporating a planning model has a side effect of leading to richer plans than those performed by human players. As such, this approach makes it difficult to use supervised generation models trained on typically egocentric human players.
%Preliminary results on human evaluation show that a templated generation system is difficult for human partners to understand.
%To circumvent this issue we develop a more robust templated generation system to take advantage of the planning system.

%\justin{new}
% Our focus lies in inference rather than models.
...
%the dialogue planner wins 85.2\% of games, compared to 77\% with a 

\begin{comment}
\justin{old}
%\textbf{Experiments}
%\justin{As our third contribution}
We demonstrate the ability of our method to prevent perspective-dependent ambiguity on \textsc{OneCommon}.
We compare models using the \emph{plans} (choices of dots to mention) they produce, evaluating whether plans resolve correctly to suitable referents in the partner context.
We find that planning
with a perspective-aware partner model
makes fewer errors than an egocentric baseline from prior work \citep{fried}.
On a static validation corpus consisting of 2,341 turns from 518 human-human dialogues, 
our proposed model achieves a 47\% reduction in the total number of incorrectly
resolved plans compared to a state-of-the-art baseline \citep{fried}.
\justin{language static eval, symbolic self-play, language self-play, human play}
\end{comment}

%\daniel{this looks good overall, especially the motivation and presentation of the setting! but the description of the contribution might not be clear to people who don't know the past work. Is there a way to concisely say what they both have in common -- planning that involves modeling the partner in some way, but past work makes the egocentric assumption and ours does not (not sure that they both model the partner in a similar-enough way, though, for this to be a helpful framing)}
