...
Large language models excel at understanding and generating text and code; however, they face limitations in adhering to task-specific constraints and handling novel grounding.
These limitations are exacerbated in grounded task-oriented dialogue.
To address these shortcomings in grounded task-oriented dialogue, we present a modular dialogue system that tackles these challenges.
We utilize a classical decomposition of the dialogue system into a reader, planner, and writer.
Our system leverages language models to convert partner utterances into executable code, incorporating grounding libraries. The translated code is executed to track dialogue state, while a symbolic planner determines the next appropriate response. We evaluate our system's performance on the demanding \textsc{OneCommon} dialogue task, which involves grounding discussions on abstract images of scattered dots. In this challenging setting, human success rate is 65.8\%. Our system demonstrates significant advancements in language understanding, as evidenced by strong results in human play, selfplay, and overall performance.
\end{abstract}
